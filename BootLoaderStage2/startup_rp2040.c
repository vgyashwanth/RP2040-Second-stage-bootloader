...
void resetHandler()
{
    ...
    uint32_t dataSize = &_edata - &_sdata;
    uint32_t * pSrc = &_sdata;
    uint32_t * pDes = &_sdataram;
    uint32_t bssSize = &_ebss - &_sbss;
    uint32_t * pbssStart = &_sbss;

    // move the .data from flash to SRAM
    for(uint32_t i = 0; i<dataSize; i++)
    {
        pDes[i] = pSrc[i];
    }

    // Initialise the .bss with 0
